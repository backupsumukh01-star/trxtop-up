...
// Validate required env before TronWeb init
if (!process.env.TRON_PRIVATE_KEY || !process.env.TRON_ADDRESS) {
    console.error('Missing TRON_PRIVATE_KEY or TRON_ADDRESS — /send-trx will fail until set in Render env');
}
...
app.post('/send-trx', validateRequest, async (req, res) => {
    try {
        ...
        const missingEnv = !process.env.TRON_PRIVATE_KEY || !process.env.TRON_ADDRESS;
        res.status(500).json({
            success: false,
            error: missingEnv ? 'Server misconfigured: set TRON_PRIVATE_KEY and TRON_ADDRESS on Render' : 'Failed to send TRX',
            message: error.message
        });
    }
});
